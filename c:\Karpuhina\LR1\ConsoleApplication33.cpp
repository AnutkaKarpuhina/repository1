...
void add(Node* tree, string str) 
{
	...
	for (int i = 0; i < str.length(); i++) //пока не будет добавлено все слово
	{
		if (ptr->next[str[i] - 'a'] != NULL) //проверка, если ли уже в словаре текущая буква
		{
			...
			else ptr = ptr->next[str[i] - 'a']; //идем вперед по дереву
		}
		else //буква не найдена, добавляем
		{
			...
			ptr = ptr->next[str[i] - 'a']; //идем вперед по дереву
		}
	}
}
...
void SearchConsEnd(Node* tree) //поиск слов, оканчивающихся на согласную
{
	for (int i = 0; i < abSize; i++)
	{
		if (tree->next[i] != NULL) 
		{
			...
			SearchConsEnd(tree->next[i]); // идем дальше по дереву
		}
	}
}
